...
for i in range(1, 48):
  ...
  for file in [f for f in os.listdir(data_dir+'/train/') if re.match(r'{}_[0-9]+\.tif'.format(i), f)]:
    ...
    if mask_not_blank(im_mask):
      ...
      numpy_image(raw_img,save_to=data_dir+"/output/{}_raw.png".format(filename))
        
  # print("\n")
  # t = np.sum(np.dstack(t), axis=2)
  t = np.mean(np.dstack(t), axis=2)
  t_mask = np.mean(np.dstack(t_mask), axis=2)
  ...
  numpy_image(t,save_to=data_dir+"/means/{}_raw.png".format(i))

  p=[]
  p = np.sum(np.dstack([t,t_mask]), axis=2)
  # plot_image(i_msk, title="mask patient: {}".format(i), save_to=data_dir+"/output/patient_{}.png".format(i))

  # m=image_with_mask(t, t_mask)
  # numpy_image(image_edges(t), title="patient: {}".format(i), save_to=data_dir+"/output/patient_{}.png".format(i))
  # numpy_image(image_edges(t_mask), title="patient mask: {}".format(i), save_to=data_dir+"/output/patient_{}_mask.png".format(i))
